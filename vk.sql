...
DROP TABLE IF EXISTS applications;  -- Сделал по анологии с сообществами,т.кподумал что в игру играет не один человек и у каждого отдельного пользователя есть свое "сохранение", не знаю в каком формате игра записывает данные о "сохранении", поэтому выбрал VARCHAR. 
CREATE TABLE applications (
	id SERIAL PRIMARY KEY,
	name VARCHAR(50),
	save VARCHAR(255),

	INDEX applications_name_idx(name)
);

DROP TABLE IF EXISTS users_applications;
CREATE TABLE users_applications (
	user_id BIGINT USIGNED NOT NULL,
	application_id BIGINT USIGNED NOT NULL,
...
DROP TABLE IF EXISTS music;  -- С музыкой тоже далеко не все понятно:) по аналогии с "вконтакте" предпологаю у музыки должна быть таблица "музыка", таблица "альбом".
CREATE TABLE music (
	id SERIAL PRIMARY KEY,
	song_title VARCHAR(40),
	song_lenght SMALLINT, -- сделал SMALLINT потому что думаю что данные о длине песни если и записываются то в секундах.
